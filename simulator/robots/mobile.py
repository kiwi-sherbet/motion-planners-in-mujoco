...
class MobileArmModel(RobotModel):
    """
    ...
    def __init__(self, base: MobileBaseModel, arms):
        ...
        if self.arm_type == "single":
            self.add_arm(arms, arm_name='right', base_name=self.base_name)
            hand_element = find_elements(
                root=arms.root, tags="body", attribs={"name": arms.eef_name}, 
                return_first=True
            )
            ...
            self.hand_rotation_offset = {}
            for arm in ("right", "left"):
                assert arm in arms, "Bimanual robot must have both arms specified"
                # print(arms[arm].arm_type)
                assert arms[arm].arm_type == 'single', "Each arm must be a single arm"
                self.add_arm(arms[arm], arm_name=arm, base_name=self.base_name[arm])
                hand_element = find_elements(
                    root=arms[arm].root, tags="body", attribs={"name": arms[arm].eef_name},
                    return_first=True
                )
                self.hand_rotation_offset[arm] = string_to_array(hand_element.get("quat", "1 0 0 0"))[[1, 2, 3, 0]]
    ...
    def _set_key_map(self):
        ...
        for key in self.base.key_map['joint'].keys():
            self._key_map['joint'].update({'base'+'_'+key: self.base.key_map['joint'][key]})
            self._key_map['actuator'].update({'base'+'_'+key: self.base.key_map['actuator'][key]})
        for arm_name, arm in self.arms.items():
            for key in arm.key_map['joint'].keys():
                self._key_map['joint'].update({arm_name+'_'+key: arm.key_map['joint'][key]})
                self._key_map['actuator'].update({arm_name+'_'+key: arm.key_map['actuator'][key]})


    def add_arm(self, arm, arm_name, base_name=None):
        """
        Mounts @gripper to arm.
        ...
        self.merge(arm, merge_body=base_name)
    ...
    def base_name(self):
        ...
        Returns:
            str or dict of str: Prefix-adjusted eef name for this robot. If bimanual robot, returns {"left", "right"}
                keyword-mapped eef names
        """
        return self.correct_naming(self._base_name)
    ...
    def default_gripper(self):
        ...
        """
        if self.arm_type == "single":
            arm = self.arms.values()[0]
            return arm.default_gripper
        else:
            return {key: arm.default_gripper for key, arm in self.arms.items()}

    @property
    def default_controller_config(self):
        if self.arm_type == "single":
            arm = self.arms.values()[0]
            return arm.default_controller_config
        else:
            return {key: arm.default_controller_config for key, arm in self.arms.items()}
    ...
    def _eef_name(self):
        ...
        if self.arm_type == "single":
            arm = self.arms.values()[0]
            return arm.naming_prefix+arm._eef_name
        else:
            return {key: arm.naming_prefix+arm._eef_name for key, arm in self.arms.items()}
